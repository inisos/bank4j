...
public class Iso20022ReferenceElementValidator {
    ...
    static {
        ...
        DEFAULT_REPLACEMENTS.put(' ', '_');
    }

    public static boolean isValidCharacterSet(String input) {
        return isValidCharacterSet(input, false);
    }
     ...
     */
    public static boolean isValidCharacterSet(String input, boolean allowDiacritics) {
        if (input == null || input.isEmpty()) {
            return false;
        }

        String inputWithoutDiacritics = StringUtils.removeDiacritics(input);
        ...
        return !(ISO20022_VALID_CHARS_PATTERN.matcher(input).find() 
                ...
                || (!allowDiacritics && !input.equals(inputWithoutDiacritics));
    }
     ...
     * 1. Remove all diacritics from input
     * 2. Any characters in the input string that do not match the ISO20022 valid characters set are replaced by a character specified in the replacements map. 
     * 3. If no replacements are provided for a specific character, the default replacement character "DEFAULT_INVALID_CHAR_REPLACEMENT" is used. 
    ...
    public static String sanitizeToCharacterSet(String input, Map<Character, Character> replacements) {
        input = StringUtils.removeDiacritics(input);
